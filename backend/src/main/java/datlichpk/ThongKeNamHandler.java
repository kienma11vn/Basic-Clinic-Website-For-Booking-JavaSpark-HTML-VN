...
public class ThongKeNamHandler {
    ...
    public static void initRoutes() {
        // API 1: Thống kê số liệu theo năm
        get("/api/admin/thong-ke-nam", (req, res) -> {
            String namStr = req.queryParams("nam");
            ...
            try (Connection conn = DriverManager.getConnection(DB_URL, USER, PASS)) {
                String sql = "SELECT "
                        ...
                        + "FROM DatLich WHERE YEAR(ngayDat) = ?";
        ...
        // API 2: Xuất báo cáo năm (.txt)
        get("/api/admin/xuat-bao-cao-nam", (req, res) -> {
            String nam = req.queryParams("nam");

            StringBuilder content = new StringBuilder();
            content.append("BÁO CÁO THỐNG KÊ NĂM: ").append(nam).append("\n");
            content.append("---------------------------\n");
            ...
            try (Connection conn = DriverManager.getConnection(DB_URL, USER, PASS)) {
                ...
                PreparedStatement pstmt = conn.prepareStatement(sql);
                pstmt.setInt(1, Integer.parseInt(nam));
                ResultSet rs = pstmt.executeQuery();

                if (rs.next()) {
                    content.append("- Tổng số ca đặt trong năm: ").append(rs.getInt("tong")).append("\n");
                    content.append("- Tổng ca đã đến khám: ").append(rs.getInt("den")).append("\n");
                    content.append("- Tổng ca đã hủy: ").append(rs.getInt("huy")).append("\n");
                }

                res.header("Content-Disposition", "attachment; filename=BaoCaoNam_" + nam + ".txt");
