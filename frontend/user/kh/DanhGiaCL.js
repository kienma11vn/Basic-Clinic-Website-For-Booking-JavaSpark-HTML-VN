...
function loadCompletedBookings(username) {
    ...
    fetch(`http://localhost:9999/api/kh/lich-da-kham?tenTK=${username}`)
        ...
        .then(data => {
            ...
            data.forEach(item => {
                const hasCL = item.danhGia && item.danhGia.includes("[CL-");
                const statusText = hasCL 
...
function openRatingModal(maDL, oldRating) {
    ...
    if (oldRating && oldRating.includes("[CL-")) {
        // Regex tìm nội dung giữa dấu ':' và dấu ']'
        const commentMatch = oldRating.match(/\[CL-\d+đ:\s*(.*?)\]/);
        if (commentMatch && commentMatch[1]) {
            oldComment = commentMatch[1].trim();
        }

        // Regex tìm số điểm (ví dụ: lấy số 10 từ [CL-10đ...])
        const scoreMatch = oldRating.match(/\[CL-(\d+)đ/);
...
function saveRating() {
    ...
    const newCLTag = `[CL-${score}đ: ${comment}]`;
    
    // LOGIC GỘP CHUỖI:
    // 1. Xóa bỏ tag [CL-...] cũ nếu có trong chuỗi oldData bằng Regex
    // 2. Giữ lại tag [CL-...] (nếu có)
    // 3. Thêm tag CL mới vào cuối
    let finalString = oldData.replace(/\[CL-.*?\]/g, "").trim(); 
    finalString = (finalString + " " + newCLTag).trim();
...
function executeSearch() {
    ...
    const filterDV = document.getElementById('filterDV').value.toLowerCase().trim();
    ...
    for (let i = 1; i < tr.length; i++) {
        ...
        if (td.length > 0) {
            ...
            if (filterDV && !txtDV.includes(filterCL)) showRow = false;
